  ...
  <header>
    <div class="wrap top">
      ...
      <nav class="nav">
        ...
        <a class="pill" href="/ui/servers/new" title="Добавить сервер вручную в БД">Добавить сервер</a>
        <a class="pill" href="/ui/alert-rules" title="Управление правилами алёртов">Алёрт-правила</a>
        <a class="pill" href="/ui/alerts" title="Просмотр активных и решённых алёртов">Алёрты</a>
        <a class="pill" href="/ui/agent-config" title="Сгенерировать ENV для агента">Конфиг агента</a>
      </nav>
    </div>
  </header>
  <main class="wrap">
...
def dashboard(request: Request, db: Session = Depends(get_db)) -> str:
    ...
    <div class="grid cols-2">
      ...
      <section class="card">
        ...
        <div class="row">
          <a class="btn" href="/docs" title="Swagger/OpenAPI: проверить запросы к API">API документация</a>
          <a class="btn" href="/ui/agent-config" title="ENV для агента: API URL/ключ/имя сервера">Сгенерировать конфиг агента</a>
        </div>
      </section>
    </div>
    """
    ...
    <section class="card">
      ...
      <form method="post" action="/ui/servers/new">
        <div class="row cols-2">
          ...
          <label>Окружение<br/><input name="environment" placeholder="prod" value="prod" /><div class="hint">Короткая метка окружения (например, prod/stage/dev) для фильтрации и понимания контекста.</div></label>
        </div>
        <label style="display:block;margin-top:10px">Метки (JSON, опционально)<br/>
          ...
          <div class="hint">Произвольные ключ‑значение для поиска/группировки серверов (например, role, zone, team).</div>
        </label>
        <div style="margin-top:10px">
          <button class="btn" type="submit" title="Создать запись сервера в БД">Создать</button>
          <a class="pill" href="/ui" title="Вернуться к списку серверов">Назад</a>
        </div>
      </form>
    </section>
    """
    ...
    <section class="card">
      ...
      <form method="post" action="/ui/alert-rules/new">
        ...
        <div class="row cols-2" style="margin-top:10px">
          <label>Имя метрики<br/><input name="metric_name" placeholder="cpu_percent" required /><div class="hint">Точное имя метрики, которую присылает агент (например, cpu_percent).</div></label>
        ...
        <div class="row cols-2" style="margin-top:10px">
          <label>Порог<br/><input name="threshold" type="number" step="0.01" value="80" required /><div class="hint">Значение, при котором сработает правило вместе с выбранным оператором.</div></label>
        ...
        <div class="row cols-2" style="margin-top:10px">
          <label>Включено<br/>
            ...
            <div class="hint">Выключенное правило хранится в системе, но не участвует в проверках.</div>
          </label>
          <label>Метки (JSON, опционально)<br/>
            <input name="labels" placeholder='{{"severity":"warning"}}' />
            <div class="hint">Доп. фильтры/контекст правила (например, severity, team), если используете метки.</div>
          </label>
        </div>
        <div style="margin-top:10px">
          <button class="btn" type="submit" title="Сохранить правило алёрта">Создать</button>
          <a class="pill" href="/ui/alert-rules" title="Вернуться к списку правил">К списку правил</a>
        </div>
      </form>
    </section>
    """
...
def alert_rules_page(deleted: str | None = None, db: Session = Depends(get_db)) -> str:
    ...
    rows = "".join(
        ...
        "<button class='btn' type='submit' title='Удалить правило алёрта'>Удалить</button>"
        "</form>"
        "</td>"
        "</tr>"
        for r in rules
    ...
    <section class="card">
      ...
      <div style="margin-bottom:10px">
        <a class="btn" href="/ui/alert-rules/new" title="Новое правило на основе метрики и порога">Создать правило</a>
      </div>
      <table>
      ...
      <div class="hint" style="margin-top:10px">Правила проверяются на сервере по последним метрикам и создают/закрывают алёрты.</div>
    </section>
    """
...
def alerts(evaluated: str | None = None, status: str | None = None, db: Session = Depends(get_db)) -> str:
    ...
    rows = "".join(
        ...
        f"<td>{html.escape(a.message)}</td>"
        f"<td class='muted'>{html.escape(_fmt_dt(a.created_at))}</td>"
        f"<td>{'' if a.status == 'resolved' else f'''<form method="post" action="/ui/alerts/{a.id}/resolve" style="display:inline"><button class="btn" type="submit" title="Пометить алёрт как решённый">Решено</button></form>'''}</td>"
        "</tr>"
        ))(*_alert_status_label(a.status))
        for a in items
    )
    ...
    <section class="card">
      <h2>Алёрты</h2>
      <div class="hint">Фильтр: <a class="pill" href="/ui/alerts" title="Показать все алёрты">все</a> <a class="pill" href="/ui/alerts?status=firing" title="Только активные алёрты">активные</a> <a class="pill" href="/ui/alerts?status=resolved" title="Только решённые алёрты">решённые</a></div>
      <div style="margin-top:10px">
        <form method="post" action="/ui/alerts/evaluate" style="display:inline">
          <button class="btn" type="submit" title="Пересчитать правила и обновить алёрты">Запустить проверку правил (MVP)</button>
        </form>
      </div>
      {banner}
      ...
      <div class="hint" style="margin-top:10px">Кнопка “Запустить проверку правил (MVP)” вручную пересчитывает правила и обновляет список алёртов.</div>
    </section>
    """
...
def agent_config_form(
    ...
    <section class="card">
      ...
      <form method="get" action="/ui/agent-config">
        ...
        <div style="margin-top:10px">
          <button class="btn" type="submit" title="Собрать ENV-переменные под агента">Сгенерировать</button>
        </div>
      </form>
    </section>
...
{server_name_line}set AGENT_DISK_PATH={html.escape(disk_path)}</code></pre>
      <div style="margin-top:10px">
        <a class="pill" href="/ui/agent-config/env?api_base_url={html.escape(api_base_url)}&api_key={html.escape(api_key)}&server_name={html.escape(server_name)}&disk_path={html.escape(disk_path)}" title="Скачать файл .env с теми же значениями">Скачать как .env</a>
      </div>
    </section>
    """
